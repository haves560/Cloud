...
- (void)setTimeBroadcastView
{
    ...
    //init NSMutableNarray
    name_list = [[NSMutableArray alloc] init];
    username_list =[[NSMutableArray alloc] init];
    ...
    adderss_list = [[NSMutableArray alloc] init];
...
//    NSInteger dayInt = [dateString substringWithRange:NSMakeRange(6, 2)].integerValue;
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@年%d月",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt];
...
//    [self setSecondScrollView];
    ...
    UIButton * OK_Bt = [[UIButton alloc] init];
    OK_Bt.frame = CGRectMake(110, 340, 100, 80);
    [OK_Bt setTitle:@"確認" forState:UIControlStateNormal];
    [OK_Bt addTarget:self action:@selector(showDateView) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:OK_Bt];
...
-(void)setTabelView
{
    ...
    tableView.frame = CGRectMake(10, 40, 300, 400);
    tableView.backgroundColor = [UIColor whiteColor];
    
    [self addSubview:tableView];
    UIImageView * titleImg = [[UIImageView alloc] init];
    
    titleImg.frame =CGRectMake(0, 0, 300, 45);
    
    titleImg.image = [UIImage imageNamed:@"title_2"];
    
    [tableView addSubview:titleImg ];
//    UILabel * titleLb = [[UILabel alloc] init];
//    
//    titleLb.frame = CGRectMake(0, 0, 300, 30);
//    titleLb.text = @"參與活動記錄";
//    [titleLb setFont:[UIFont systemFontOfSize:20]];
//    [tableView addSubview:titleLb];
    
    tableview = [[UITableView alloc] initWithFrame:CGRectMake(0, 50, 300, 300)];
    ...
    closeBt.frame = CGRectMake(90, 330, 100, 60);
    [closeBt setTitle:@"關閉" forState:UIControlStateNormal];
    [closeBt addTarget:self action:@selector(closeView) forControlEvents:UIControlEventTouchUpInside];
    closeBt.backgroundColor = [UIColor colorWithRed:166.0/255.0 green:210.0/255.0 blue:237.0/255.0 alpha:1];
    [tableView addSubview:closeBt];
    
    [tableview registerNib:[UINib nibWithNibName:@"QRTableViewCell" bundle:nil] forCellReuseIdentifier:@"cell"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //NSLog(@"date = %@ , checkdate = %@",date_list,_checkDate);
        ...
        cell.timeLb.text = [NSString stringWithFormat:@"%@ %@",[date_list objectAtIndex:indexPath.row],[time_list objectAtIndex:indexPath.row]];
        cell.address.text = [NSString stringWithFormat:@"%@%@%@",[city_list objectAtIndex:indexPath.row],[area_list objectAtIndex:indexPath.row],[adderss_list objectAtIndex:indexPath.row]];
        cell.detail = [detail_list objectAtIndex:indexPath.row];
    
        NSString *propaganda_url = [NSString stringWithFormat:@"http://172.20.10.2/ClubCloud/userServer/Request/request_img/%@.png",[data_id_list objectAtIndex:indexPath.row]];
        NSURL *imageURL = [NSURL URLWithString:propaganda_url];
        NSData *imageData = [NSData dataWithContentsOfURL:imageURL];
        UIImage *image = [UIImage imageWithData:imageData];
    
        cell.colorIcon.image = image;
...
-(void)showDateView
{
    ...
    {
        UIAlertView * al = [[UIAlertView alloc]initWithTitle:@"訊息" message:@"本月份無活動！" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
...
{
    yearScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(73, 0, 73.0, 190.0)];
...
{
    monthScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(150.0, 0, 40.5, 190.0)];
...
- (void)scrollviewDidChangeNumber
{
    ...
    //NSString *weekString = [self fromDateToWeek:dateString];
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%d年%d月",yearInt,monthInt];
    
    NSString * yearAndMonth = [NSString stringWithFormat:@"%d/%d",yearInt,monthInt];
...
-(void)selectdb
{
    [name_list removeAllObjects];
    [username_list removeAllObjects];
    ...
    if([db open]){
        ...
        NSString * sqlQuery = [NSString stringWithFormat:@"SELECT * FROM '%@' where %@ = '%@'  ",@"QRdb",@"month",month];
        NSLog(@"%@",sqlQuery);
        FMResultSet *rs=[db executeQuery:sqlQuery];
        
        while ([rs next])
        {
            NSString * _name = [rs stringForColumn:@"name"];
            
            [name_list addObject:_name];
            
            NSString * _username = [rs stringForColumn:@"username"];
            
            [username_list addObject:_username];
            
            NSString * _title = [rs stringForColumn:@"title"];
            
            [title_list addObject:_title];
            
            NSString * _detail = [rs stringForColumn:@"detail"];
            
            [detail_list addObject:_detail];
            
            NSString * _date = [rs stringForColumn:@"date"];
            
            [date_list addObject:_date];
            
            NSString * _time = [rs stringForColumn:@"time"];
            
            [time_list addObject:_time];
            
            NSString * _city  =[rs stringForColumn:@"city"];
            
            [city_list addObject:_city];
            
            NSString * _area = [rs stringForColumn:@"area"];
            
            [area_list addObject:_area];
            
            NSString * _address = [rs stringForColumn:@"address"];
            
            [adderss_list addObject:_address];
            
            NSString * _image = [rs stringForColumn:@"image"];
            
            [image_list addObject:_image];
            
            NSString * _month = [rs stringForColumn:@"month"];
            NSLog(@"%@",_month);
            
            NSString * _data_id = [rs stringForColumn:@"data_id"];
